...
+ This uses the Youtube Data Api. Data must be collected over the course of days, since the youtube data api has a daily quota. _get_data.py_ is to be run each day with renewed quota until all data has been collected. The key given will be used to exhaustion each day unless there is no more data. **Do not change the key and run before cleaning the state. This may result in key getting banned from Youtube.**
